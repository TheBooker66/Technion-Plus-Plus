			...
			new Promise((resolve, reject) => {
				(async () => {
					...
					if (chrome.runtime.lastError) {
						console.error(
							"TE_ww_cal: " + chrome.runtime.lastError.message,
						);
						reject({
					...
					for (const course of Object.values(
						...
						courseMap[(course as WebworkCourse).lti] = (
							course as WebworkCourse
						).name;

					const date = new Date(storageData.webwork_cal_update);
					document.getElementById("last_check")!.style.display =
						"block";
					document.getElementById("last_check")!.textContent +=
						storageData.webwork_cal_update
					...
					Object.keys(webworkCalendarData).forEach((assignment) => {
						sortedAssignments.push([
							parseInt(assignment),
							webworkCalendarData[assignment],
						]);
					});
					sortedAssignments.sort((a, b) => {
						return (
							a[1].ts - b[1].ts || a[1].h.localeCompare(b[1].h)
						);
					});
					for (let i = 0; i < sortedAssignments.length; i++) {
						const assignment = sortedAssignments[i];
						const courseLTI = assignment[0]
							.toString()
							.substring(
								0,
						...
						const assignmentObject: HWAssignment = {
							...
							goToFunc: () =>
								chrome.tabs.create({
									url: `https://moodle25.technion.ac.il/mod/lti/launch.php?id=${courseLTI}`,
								}),
							eventID: eventID,
						...
						if (assignment[1].done)
							finishedAssignmentsList.push(assignmentObject);
