...
async function setupListDownloadButtons(
	...
	const pollingInterval = setInterval(() => {
		if (
			(document.getElementById("loadingMessage") as HTMLElement).style.display !== "none" ||
		...
		for (const tableRow of tableRows) {
			...
			downloadLink.addEventListener("click", async () => {
				await chrome.runtime.sendMessage({
					...
					link: `https://panoptotech.cloud.panopto.eu/Panopto/Podcast/Syndication/${tableRow.getAttribute("id")}.mp4`,
			...
			titleElement.append(downloadLink);
			...
			titleElement.append(checkboxLabel);
		}
	}, 2e3);
}
...
async function setupFolderDownloadButtons() {
	...
	downloadAllLink.addEventListener("click", async () => {
		...
		for (const xmlItem of xmlItems) {
			...
			downloadItem.u = xmlItem.querySelector("guid")!.textContent.split("/Syndication/")[1];
			downloadItem.n = `${
				courseTitle +
				xmlItem
					.querySelector("title")!
					.textContent.replace(/[^a-zA-Z\u05d0-\u05ea0-9\- ]/g, "")
					.replace(/\s\s+/g, " ")
			}.mp4`;
	...
	downloadSelectedLink.addEventListener("click", async () => {
		...
		for (const listRow of listRows) {
			if (!(listRow.querySelector(".tplus_check") as HTMLInputElement).checked) continue;
			const downloadItem = {
				u: `${listRow.id}.mp4`,
				n: `${
					courseTitle +
...
הוידאו נפתח לתצוגה בחלון חדש, יש לסגור את החלון החדש כדי לחזור ולצפות בווידאו כאן.
				...
				secondaryScreen.prepend(newWindowMessageDiv);
				...
				expandButton.addEventListener("click", () => {
					...
					newDocument.body.append(newWindowCanvas);
					...
					newWindowCanvas.setAttribute(
						"style",
						"{max-width: 800px; border: 1px solid #fff; margin: auto; display: block;}".replace(/[{}]/g, "")
					);
					...
					newDocument.addEventListener("dblclick", async () => {
						if (!NewWindowFullscreenEh) return;
						await newDocument.exitFullscreen();
						NewWindowFullscreenEh = false;
					});
					fullscreenButton.addEventListener("click", async () => {
						await newWindowCanvas.requestFullscreen();
						NewWindowFullscreenEh = true;
					});
					newDocument.body.append(fullscreenButton);
	...
	switch (settingsKey) {
		...
		case "floating_speed":
			settingsObj.panopto_floating_speed = (
				document.getElementById("m_floating_speed") as HTMLInputElement
			).checked;
			break;
		case "scroll_volume":
		...
		case "show_hide_thumbnails": {
			const attr = (document.getElementById("toggleThumbnailsButton") as HTMLDivElement).getAttribute(
				"aria-expanded"
			);
		...
		case "show_hide_sidebar": {
			const attr = (
				document.querySelector("#eventsExpanderButton > div[role=button]") as HTMLDivElement
...
function updateRealTime() {
	...
	else (document.getElementById("tplusRealTimeRemaining") as HTMLDivElement).textContent = timeString.slice(3);
...
function setupTimer() {
	...
	newTimeElement.setAttribute(
		"style",
		"{vertical-align: middle; width: 50px; font-size: 0.95em;}".replace(/[{}]/g, "")
	);
	...
	(document.getElementById("transportControls") as HTMLDivElement).insertBefore(
		newTimeElement,
		document.getElementById("liveButton")
	);
	(document.getElementById("primaryVideo") as HTMLVideoElement).addEventListener("timeupdate", () =>
		updateRealTime()
	);
}
...
function applyCustomSpeed(speedValue: string) {
	...
	for (const video of document.querySelectorAll(".video-js")) (video as HTMLVideoElement).playbackRate = parsedSpeed;
}
...
`,
	...
	bigBossElement.append(menu);
	(document.getElementById("tplus_koteret") as HTMLDivElement).style.backgroundImage =
	...
	(document.getElementById("m_floating_speed") as HTMLInputElement).addEventListener("change", async () => {
		if ((document.getElementById("m_floating_speed") as HTMLInputElement).checked)
			setupFloatingSpeedController(
				document.fullscreenElement ? (document.fullscreenElement as HTMLElement) : document.body
			);
		else document.getElementById("tplus_floating_speed_controller")?.remove();
		await saveSetting("floating_speed");
	});

	(document.getElementById("m_return_backwards") as HTMLInputElement).addEventListener("change", () =>
		saveSetting("return_backwards")
	);
	(document.getElementById("m_scroll_volume") as HTMLInputElement).addEventListener("change", () =>
		saveSetting("scroll_volume")
	);
	(document.getElementById("m_save") as HTMLInputElement).addEventListener("change", () => saveSetting("settings"));

	if (
		document.pictureInPictureEnabled &&
		!(document.querySelector(".video-js") as HTMLVideoElement).disablePictureInPicture
	) {
...
function setupFloatingSpeedController(parentElement: HTMLElement = document.body) {
	...
	parentElement.append(floatingPanel);
	...
	floatingPanel.append(speedInput);
...
async function fullScreenToggleHandler(event: KeyboardEvent) {
	...
	else mainElement.dispatchEvent(new MouseEvent("dblclick", {bubbles: true, cancelable: true, view: window}));
}
...
async function setupVideoPage() {
	...
	sidebarButton?.addEventListener("click", async () => await saveSetting("show_hide_sidebar"));
	setTimeout(() => {
		if (storageData.panopto_hide_thumbnails && thumbnailsButton?.style.display !== "none") thumbnailsButton.click();
		if (
