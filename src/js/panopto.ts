...
הוידאו נפתח לתצוגה בחלון חדש, יש לסגור את החלון החדש כדי לחזור ולצפות בווידאו כאן.
	...
	async function copyTimestampedURL(markdownLinkEh: boolean) {
		...
		if (!markdownLinkEh) await navigator.clipboard.writeText(timestampUrl);
		else {
			let resultText = "";
...
`,
	...
	if (window.location.href.includes("List.aspx")) await setupFolderDownloadButtons();
		...
		if (storageData.panopto_save) {
			...
			(document.getElementById("m_floating_speed") as HTMLInputElement).checked =
				storageData.panopto_floating_speed;
			(document.getElementById("m_scroll_volume") as HTMLInputElement).checked =
				storageData.panopto_scroll_volume;
