...
(async function () {
	...
	if (
		...
		// Error colours
		(document.getElementById("bus_error") as HTMLDivElement).style.display =
			"block";
	await resetBadge();
	...
	screenTransitions.forEach((linkObject) => {
		document
			.getElementById(linkObject.button)!
	...
	const popup_window = {
		...
		toolLinks = (
			document.getElementById("tools_content") as HTMLDivElement
		).querySelectorAll("a");
	popup_window.top = parseInt(
		(
			(window.screen.height - (popup_window.height as number)) /
			2
		).toString(),
	);
	popup_window.left = parseInt(
		((window.screen.width - (popup_window.width as number)) / 2).toString(),
	);
	(
	...
	for (let i = 0; i < toolLinks.length; i++) {
		...
		toolLinks[i].addEventListener(
			"click",
			() =>
				(window.location.href =
					"../html/p_" + toolLinks[i].id + ".html"),
		);
	}

	const mainScreensLinks = document.querySelectorAll(
			"#tools_and_links > div",
		) as NodeListOf<HTMLDivElement>,
		printTabs = (
			document.getElementById("print_tabs") as HTMLDivElement
	...
	makeTabsClicky(
		mainScreensLinks[0].querySelectorAll(".tab"),
		Array.from(mainScreensLinks).slice(1),
	);
	makeTabsClicky(printTabs, pages);

	(
		(document.getElementById("cant_login") as HTMLDivElement).querySelector(
			"u",
		) as HTMLElement
	...
	const quick_login_toggle = document.getElementById(
			"quick_login_toggle",
		) as HTMLInputElement,
		mute_alerts_toggle = document.getElementById(
			"mute_alerts_toggle",
		) as HTMLInputElement;
	quick_login_toggle.addEventListener("change", async () => {
		await chrome.storage.local.set({
			quick_login: quick_login_toggle.checked,
		});
		if (chrome.runtime.lastError)
			console.error(
				"TE_popup_login: " + chrome.runtime.lastError.message,
			);
	});
	mute_alerts_toggle.addEventListener("change", async () => {
		await chrome.storage.local.set({
			alerts_sound: mute_alerts_toggle.checked,
		});
		if (chrome.runtime.lastError)
			console.error(
				"TE_popup_mute_alerts: " + chrome.runtime.lastError.message,
			);
	});
	...
	(
		document.getElementById("cal_webwork") as HTMLAnchorElement
	).style.display =
		storageData.enable_login &&
		storageData.quick_login &&
	...
	for (let i = 0; i < calendarIDs.length; i++) {
		...
		if (storageData.cal_seen & Math.pow(2, i))
			(
				document.getElementById(calendarIDs[i]) as HTMLDivElement
			).className = "major hw";
	}

	if (storageData.dl_current !== 0)
		(
			document.getElementById("downloads") as HTMLAnchorElement
	...
	for (let i = 0; i < printerLinks.length; i++) {
		...
		printerLinks[i].addEventListener("click", async () => {
			...
			if (chrome.runtime.lastError)
				console.error(
					"TE_popup_printers: " + chrome.runtime.lastError.message,
				);
		});
	}

	const studentsLink = document.getElementById(
			"UGS_Link",
		) as HTMLAnchorElement,
		customLink = document.getElementById(
			"custom_link",
		) as HTMLAnchorElement;
	studentsLink.addEventListener("click", async () => {
		const loadingInterval = setInterval(() => {
			studentsLink.textContent =
				7 > studentsLink.textContent.length
					? studentsLink.textContent + "."
					: "טוען";
		}, 500);
		let authURL;
		try {
			authURL = (
				await fetch("https://students.technion.ac.il/auth/oidc/", {
					method: "HEAD",
				})
			).url;
		} catch {
		...
		urlParams.append(
			...
			storageData.username +
				"@" +
				(storageData.email_server ? "campus." : "") +
				"technion.ac.il",
		);
		await chrome.tabs.create({
			url: urlParts[0] + "?" + urlParams.toString() || authURL,
		});
