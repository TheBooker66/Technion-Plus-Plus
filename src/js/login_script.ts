(async function () {
	type ExtendedStorageData = StorageData & {
		username_ext: string;
		password: string;
	};
	...
	function microsoft(storageData: ExtendedStorageData) {
		const handleMicrosoftLogin = () => {
			...
			if (
				...
				(location.pathname.includes("/oauth2/authorize") ||
					location.pathname.includes("/saml2"))
			) {
				const SelectAccountPageEh =
					location.href.includes("select_account");
				const LoginHintPresentInUrlEh =
					location.href.includes("login_hint");
				if (
					loginForm["passwd"] ||
				...
				if (
					...
					location.href ===
						"https://login.microsoftonline.com/f1502c4c-ee2e-411c-9715-c855f6753b84/login"
				) {
		...
		if (document.querySelector(".banner-logo")) handleMicrosoftLogin();
			...
			new MutationObserver(() => handleMicrosoftLogin()).observe(
				document.forms[0] ?? document.body,
	...
	function techwww(storageData: ExtendedStorageData) {
		...
		form.setAttribute(
			"action",
			"https://moodle.technion.ac.il/login/index.php",
		);
	...
	function moodle(storageData: StorageData, website: string) {
		if (
			...
			!document.querySelector(".usermenu > .login")
		)
			return;
		if (storageData.external_enable)
			window.location.href = "https://techwww.technion.ac.il/tech_ident/";
		else {
			const specificMoodleNumber =
				website.match(/moodle([0-9]+)\.technion\.ac\.il/)?.[1] || "25";
	...
	function cs(storageData: ExtendedStorageData) {
		if (
			document.querySelector("form")!.querySelectorAll(".red-text")
				.length !== 0 ||
			document.referrer.includes("grades.cs.technion.ac.il")
		)
			return;
		...
		if (
			document.querySelector("form")!.querySelectorAll(".white-button")
				.length > 0
		)
			...
			(
				document
					.querySelector("form")!
					.querySelector(".submit-button") as HTMLButtonElement
			).click();
	}

	function sap(storageData: ExtendedStorageData) {
		if (
			(document.getElementById("certLogonForm") === null ||
				document.referrer.includes("portalex")) &&
	...
	function panopto() {
		if (
			document.getElementById("loginButton") != null &&
			window.location.href.includes("Pages/Home.aspx")
		)
			window.location.href =
				(-1 === window.location.href.indexOf("?") ? "?" : "&") +
				"instance=TechnionAuthentication";
		if (
			document.getElementById(
				"PageContentPlaceholder_loginControl_externalLoginButton",
			) != null &&
			((
				document.getElementById("providerDropdown") as HTMLInputElement
			).value = "TechnionAuthentication")
		)
			(
				document.getElementById(
					"PageContentPlaceholder_loginControl_externalLoginButton",
	...
	if (storageData.enable_login && !storageData.external_enable) {
		storageData.password = reverseString(
			xorStrings(
		...
		if (/moodle[0-9]*.technion.ac.il/.test(website))
			...
			website === "grades.cs.technion.ac.il" ||
			website === "webcourse.cs.technion.ac.il"
		) {
			cs(storageData);
		...
		storageData.username_ext = reverseString(
			xorStrings(storageData.uidn_arr[0] + "", storageData.uidn_arr[1]),
		);
		if (/moodle[0-9]+.technion.ac.il/.test(website))
			moodle(storageData, website);
