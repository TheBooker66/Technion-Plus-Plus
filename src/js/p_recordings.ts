...
(async function () {
	...
	function processSearchResults(
		...
		if (matchCount === 1) displayCourseRecordings(resultsArray[0]);
		...
		const callbacks = [
			...
			() =>
				processSearchResults(
					...
					storageData.videos_data as {[p: string]: RecordingCourse["v"]},
					courseQuery
				),
		];
	...
	if (courseQuery) {
		...
		if (courseQuery.length === 0) displayError("לא ניתן לשלוח בקשה ריקה, נסה שנית.");
			...
			else processSearchResults(storageData.videos_courses, storageData.videos_data, courseQuery as string);
		}
	} else {
