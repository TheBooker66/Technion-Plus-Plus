(async function () {
	async function main() {
		...
		async function create_tp_buttons() {
			const section = create_element(
				...
				"block block_material_download card mb-3 tplus_block",
				{},
				"",
				true,
			) as HTMLElement;
			const cardBody = create_element(
				"div",
				section,
				"card-body",
			) as HTMLDivElement;

			const cardTitle = create_element(
				"h5",
				cardBody,
				"card-title d-inline",
				{
			...
			const actionsContainer = create_element(
				"div",
				cardBody,
				"card-text mt-3 tplus_main_actions",
			) as HTMLDivElement;
			const darkModeContainer = create_element(
				"div",
				actionsContainer,
			...
			const darkModeCheckbox = create_element(
				"input",
				darkModeContainer,
			...
			const colourSliderContainer = create_element(
				"div",
				actionsContainer,
				"",
				{
			...
			const colourSlider = create_element(
				"input",
				colourSliderContainer,
				"",
				{
			...
			darkModeCheckbox.addEventListener("change", async () => {
				await chrome.storage.local.set({
					remoodle: darkModeCheckbox.checked,
				});
			...
			colourSlider.style.backgroundImage =
		...
		if (".ac.il/" === window.location.href.split("technion")[1]) {
			...
			const courseTiles = document.querySelectorAll(".coursevisible"),
				...
				courseNameRegex = /(?<cname>.+)\s-\s(?<cnum>[0-9]+)/,
				semesterRegex = / - (?:חורף|אביב|קיץ)/;
			for (const courseTile of courseTiles) {
				const courseMatch = courseTile
					.querySelector("h3")
					?.textContent.replace(semesterRegex, "")
					.match(courseNameRegex);
				if (!courseMatch?.groups) continue;
				userCourses[courseMatch.groups.cnum.trim()] =
					courseMatch.groups.cname.trim();
			}
			if (Object.keys(userCourses).length > 0) {
				await chrome.storage.local.set({
					moodle_cal_courses: userCourses,
				});
				if (chrome.runtime.lastError)
					console.error(
						"TE_moodle_001_: " + chrome.runtime.lastError,
					);
			}
			...
			if (0 < courseTiles.length)
				for (let i = 0; i < courseTiles.length; i++) {
					const downloadButtonContainer =
						document.createElement("div");
					downloadButtonContainer.style.cssFloat = "left";
					courseContainers[i].insertBefore(
						downloadButtonContainer,
						courseContainers[i].querySelector(
							"a.btn.btn-primary.coursestyle2btn",
						),
					);
					const course =
							courseTiles[i].querySelector("h3")!.textContent,
						courseLink = courseTiles[i]
			...
			document
				.getElementById("coursecontentcollapseid2")
				?.insertBefore(
			...
			(
				parentNode
					.querySelector(".tplus_main_actions")!
					.appendChild(
						parentNode.querySelector("h5") as Node,
					) as HTMLElement
			...
			const moodleNum = window.location.href.split("?id=")[1],
				course = document.title.match(
					/(?<cname>.+)\s-\s(?<csemester>.+)\s-\s(?<cnum>[0-9]+)/,
				),
			...
			if (course_num) {
				...
				const short_course_num =
					course_num.substring(1, 4) + course_num.substring(5, 8);
				let videoID = "";
				for (let i = 0; i < storageData.videos_courses.length; i++)
					if (
						storageData.videos_courses[i]
							.join(" ")
							.includes(short_course_num)
					) {
						videoID = storageData.videos_courses[i][0];
						break;
					}
				if (videoID !== "" && storageData.videos_data[videoID]) {
					const data: RecordingCourse["v"] =
						storageData.videos_data[videoID];
					for (let j = 0; j < data.length; j++) {
						let text = "";
						text = 1 < data.length ? `וידאו #${j + 1} ` : "וידאו ";
						text =
							0 < data[j]["t"]
								? ["הרצאה", "תרגול"][data[j]["t"] - 1]
								: text;
						text += "(פנופטו)";
						create_element(
							"a",
			...
			for (const element of document.querySelectorAll(
				...
				if (
					element.classList.contains("accesshide") ||
	...
	async function colourPage() {
		...
		if (chrome.runtime.lastError) {
			console.error(
				"TE_remoodle_err: " + chrome.runtime.lastError.message,
			);
			return;
		}
		...
		const setDarkMode = (darkmodeEh: boolean) => {
			...
			if (darkmodeEh) entirePage.setAttribute("tplus", "dm");
		...
		chrome.runtime.onMessage.addListener((message) => {
			switch (message.mess_t) {
				...
				case "TE_moodle_colour": {
					themeProperties(message.angle).forEach((property) =>
						document.documentElement.style.setProperty(
							property[0],
							property[1],
						),
					);
					const colorSlider = document.querySelector(
						"#tp_colour_switcher input[type=range]",
					) as HTMLInputElement;
