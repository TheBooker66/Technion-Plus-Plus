	...
	function openMoodle(
		...
		return async () => {
			try {
				...
				const eventLinks = response.response.querySelectorAll(
					`.event[data-event-id='${eventID}'] a`,
				);
	...
	async function initializeCalendarProperties(
		...
		try {
			...
			if (storageData.moodle_cal_prop === "") {
				...
				const res = await popup.XHR(
					"https://moodle25.technion.ac.il/calendar/export.php",
					"document",
				);
				const sessionKey: string =
					res.response.querySelector("[name='sesskey']").value;
				const exportResponse = await popup.XHR(
					res.responseURL,
					"document",
					"sesskey=" +
						sessionKey +
						"&_qf__core_calendar_export_form=1&events[exportevents]=all&period[timeperiod]=recentupcoming&generateurl=\u05d4\u05e9\u05d2+\u05d0\u05ea+\u05db\u05ea\u05d5\u05d1\u05ea+\u05d4-URL+\u05e9\u05dc+\u05dc\u05d5\u05d7+\u05d4\u05e9\u05e0\u05d4",
				);
				const exportResponse2: string =
					"userid=" +
				...
				await chrome.storage.local.set({
					moodle_cal_prop: exportResponse2,
				});
				if (chrome.runtime.lastError) {
		...
		() =>
			new Promise((resolve, reject) => {
				(async () => {
					...
					if (chrome.runtime.lastError) {
						console.error(
							"TE_cal: " + chrome.runtime.lastError.message,
						);
						reject({
							msg: "שגיאה בניסיון לגשת לנתוני הדפדפן, אנא נסה שנית.",
							is_error: true,
					...
					if (storageData.moodle_cal_prop === "") {
						TE_AutoLogin(true)
							.then(() => initializeCalendarProperties(reject))
							.catch(() =>
								reject({
					...
					const calendarData = await popup.XHR(
						"https://moodle25.technion.ac.il/calendar/export_execute.php?preset_what=all&preset_time=recentupcoming&" +
					...
					try {
						if (
							...
							TE_AutoLogin(true)
								.then(() =>
									initializeCalendarProperties(reject),
								)
								.catch(() =>
									reject({
						...
						const cal: string[] =
							calendarData.response.split("BEGIN:VEVENT");
						if (cal.length === 1) {
							await chrome.storage.local.set({
								cal_seen: await calendar.removeCalendarAlert(
						...
						for (let i = 1; i < cal.length; i++) {
							const eventID = parseInt(
								cal[i].split("UID:")[1].split("@moodle")[0],
							);
							maxEventID =
								eventID > maxEventID ? eventID : maxEventID;

							if (!cal[i].includes("CATEGORIES")) continue;

							const eventTitle = cal[i]
								.split("SUMMARY:")[1]
								.split("\n")[0]
								.trim();
							if (
								(storageData.filter_toggles.appeals &&
									eventTitle.includes("ערעור")) ||
								...
								(storageData.filter_toggles.attendance &&
									(eventTitle.includes("נוכחות") ||
										...
										eventTitle.includes("attendance"))) ||
								(storageData.filter_toggles.reserveDuty &&
									eventTitle.includes("מילואים"))
							)
								continue;
							...
							let eventDateStr: string = cal[i]
								.split("DTSTART")[1]
								.split("\n")[0]
								.replace(";VALUE=DATE:", "")
								.replace(":", "");
							const eventTimeStr: string = eventDateStr.includes(
								"T",
							)
								? eventDateStr
										.split("T")[1]
							...
							const eventTimeFinal = eventDate.toLocaleString(
								"he-IL",
								{
									weekday: "long",
									day: "2-digit",
							...
							const courseInfo: string[] = cal[i]
								.split("CATEGORIES:")[1]
								.split("\n")[0]
								.trim()
								.split(".");
							const courseNum = courseInfo[0]
									?.replace(/[^0-9]/i, "")
									.trim(),
								semesterNum = (courseInfo[2] ?? courseInfo[1])
									?.replace(/[^0-9]/i, "")
									.trim();
							const course =
								Object.prototype.hasOwnProperty.call(
									storageData.moodle_cal_courses,
									courseNum,
								) && semesterNum.toString() in semesters
									? storageData.moodle_cal_courses[
											courseNum
										] +
										(semesterNum
											? ` - ${semesters[semesterNum as "200" | "201" | "202"]}`
											: "")
									: courseInfo.toString();
							...
							let eventDescription: string = cal[i]
								...
								.trim();
							eventDescription =
								95 < eventDescription.length
									? eventDescription.slice(0, 90) + "..."
									: eventDescription;

							const finishedEh = (
								storageData.moodle_cal_finished as number[]
							).includes(eventID);
							...
							const event: HWAssignment = {
								...
								pinned: storageData.pinned_assignments.includes(
									eventID,
								),
							};
						...
						if (chrome.runtime.lastError)
							console.error(
								"TE_cal_moodle: " + chrome.runtime.lastError,
							);
						newAssignmentsList.sort(
							(a, b) =>
								a.timestamp - b.timestamp ||
								a.name.localeCompare(b.name),
						);
						finishedAssignmentsList.sort(
							(a, b) =>
								a.timestamp - b.timestamp ||
								a.name.localeCompare(b.name),
						);
						resolve({
							new_list: newAssignmentsList,
							finished_list: finishedAssignmentsList,
						});
					} catch (err) {
						console.error(err);
						reject({
					...
					reject({
						msg: "אירעה שגיאה פנימית, אנא נסה מאוחר יותר.",
						is_error: true,
					});
				});
			}),
