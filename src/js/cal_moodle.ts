...
(async function () {
    ...
    await calendar.progress(() => new Promise((resolve, reject) => {
        (async () => {
            ...
            if (storageData.moodle_cal_prop === "") {
                TE_AutoLogin(true).then(() => initializeCalendarProperties(reject)).catch(() => reject({
            ...
            try {
                ...
                const finishedEvents: number[] = [], newAssignmentsList: HWAssignment[] = [], finishedAssignmentsList: HWAssignment[] = [];
                let maxEventID = 0;
                for (let i = 1; i < cal.length; i++) {
                    ...
                    const eventTimeStr: string = eventDateStr.includes("T") ? eventDateStr.split("T")[1]
                    ...
                    const course = (Object.prototype.hasOwnProperty.call(storageData.moodle_cal_courses, courseNum) && semesterNum.toString() in semesters) ?
                    ...
                    if (finishedEh) finishedAssignmentsList.push(event); else newAssignmentsList.push(event);
                }
                await chrome.storage.local.set({
