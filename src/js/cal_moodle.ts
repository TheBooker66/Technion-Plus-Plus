...
(async function () {
	...
	async function initializeCalendarProperties(
		errorCallback: ({msg, errorEh}: {msg: string; errorEh: boolean}) => void
	) {
		try {
			...
			new Promise(async (resolve, reject) => {
				...
				try {
					...
					for (let i = 1; i < cal.length; i++) {
						...
						if (
							!(
								"opens" !== titleWords[titleWords.length - 1] &&
								"opens)" !== titleWords[titleWords.length - 1]
							)
						)
							continue;
						...
						const course =
							Object.hasOwn(storageData.moodle_cal_courses, courseNum) &&
						...
						let eventDescription: string = cal[i]
							...
							.trim();
						eventDescription =
							95 < eventDescription.length ? eventDescription.slice(0, 90) + "..." : eventDescription;
