(function () {
	function createDownloadButton(
		...
		downloadButton.addEventListener("click", async () => {
			...
			for (let i = 0; i < fileLinks.length; i++) {
				...
				if (webcourseEh) {
					...
					downloadItem.n =
						...
						subdirectory +
						decodeURIComponent(
							fileLinks[i][0].split("/WCFiles/")[1],
						)
							.replace(/[^a-zA-Z\u05d0-\u05ea0-9\-_. ]/g, "")
							.trim();
					downloadItem.u = decodeURIComponent(fileLinks[i][0]).split(
						"/WCFiles/",
					)[1];
				}
			...
			downloadChunk.sub_pre = webcourseEh
				? ""
				: decodeURIComponent(fileLinks[0][0])
						.split("?")[1]
						.split("/WCFiles/")[0] + "/WCFiles/";
	...
	for (let i = 0; i < allLinks.length; i++)
		if (allLinks[i].getAttribute("target") === "wc_output")
			allLinks[i].setAttribute("target", "_blank");
	if (
		window.location.href.includes("ho.html") ||
		window.location.href.search("ho_.*.html") !== -1
	)
		for (
			let ticketContainers = document.querySelectorAll(".tickets"), i = 0;
			i < ticketContainers.length;
			i++
		) {
			const pdfLinks = [],
				pptLinks = [],
			...
			for (let j = 0; j < ticketItems.length; j++) {
				const fileAnchors = ticketItems[j].querySelectorAll("a"),
					ticketTitle =
						ticketItems[j].querySelector("h2")!.textContent;
				for (let k = 0; k < fileAnchors.length; k++) {
					const fileURL = fileAnchors[k].getAttribute(
						"href",
					) as string;
					if (/Spring|Summer|Winter/i.test(fileURL)) {
						...
						else otherLinks.push([fileURL, ticketTitle]);
					}
				}
			}
			const allLinks = pdfLinks.concat(
				pptLinks,
				docLinks,
				zipLinks,
				otherLinks,
			...
			fieldset.appendChild(document.createElement("legend")).textContent =
				"Technion";
			downloadButtonsContainer.className = "tplus_flex";
			if (
				allLinks.length > pdfLinks.length &&
				allLinks.length > pptLinks.length &&
				allLinks.length > docLinks.length &&
				allLinks.length > zipLinks.length
			)
			...
			(
				ticketContainers[i].insertBefore(
					document.createElement("div").appendChild(fieldset)
						.parentNode as Node,
					ticketContainers[i].firstChild,
				) as HTMLElement
