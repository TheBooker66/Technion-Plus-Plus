...
(async function () {
	...
	await calendar.progress(() => new Promise((resolve, reject) => {
		chrome.storage.local.get({
			...
			uidn_arr: ["", ""],
			pinned_assignments: [],
		}).then(async (storageData: StorageData) => {
			...
			try {
				...
				const finishedItems: number[] = [], seenItems: { [key: string]: string } = {};
				let toDoList: HWAssignment[] = [], finishedList: HWAssignment[] = [], courseName = "";
				for (let i = 1; i < eventSections.length; i++) {
					...
					if (eventSections[i].includes(".PHW")) {
						if (dueDate.getTime() > currentTime) {
							...
							if (Object.prototype.hasOwnProperty.call(seenItems, courseName))
					...
					if (!Object.prototype.hasOwnProperty.call(seenItems, courseName)) seenItems[courseName] = "";
					seenItems[courseName] += "[[" + trimmedSummary + "]]";

					const newEventEh = !(Object.prototype.hasOwnProperty.call(storageData.cs_cal_seen, courseName) &&
					...
					if (finishedEh) finishedList.push(Assignment); else toDoList.push(Assignment);
				}
