			...
			new Promise((resolve, reject) => {
				(async () => {
					...
					if (chrome.runtime.lastError) {
						console.error(
							"TE_cs_cal: " + chrome.runtime.lastError.message,
						);
						reject({
					...
					const calendarPass = reverseString(
						xorStrings(
					...
					try {
						const responseData: {
							response: string;
							responseURL: string;
						} = await popup.XHR(calendarUrl, "text");
						const eventSections =
							responseData.response.split("BEGIN:VEVENT");
						...
						for (let i = 1; i < eventSections.length; i++) {
							const summary = eventSections[i].match(
								regexPatterns.summary,
							)![1];
							let trimmedSummary = summary.split("(")[0].trim();
							if (regexPatterns.banned.test(trimmedSummary))
								continue;

							if (
								eventSections[i].includes(
									"icspasswordexpired",
								) ||
								eventSections[i].includes("icspasswordexpired1")
							) {
								reject({
							...
							const eventUIDMatch = eventSections[i].match(
								regexPatterns.uid,
							)?.[1];
							...
							const timeMatch = eventSections[i].match(
								regexPatterns.time,
							...
							if (
								...
								dueDate.getTime() > currentTime + THIRTY_DAYS
							)
								continue;
							...
							if (eventSections[i].includes(".PHW")) {
								if (dueDate.getTime() > currentTime) {
									...
									if (
										...
										seenItems[courseName] = seenItems[
											courseName
										].replace(
											"[[" + trimmedSummary + "]]",
											"",
							...
							const newEventEh = !(
								...
								storageData.cs_cal_seen[courseName].includes(
									"[[" + trimmedSummary + "]]",
								)
							);

							const finishedEh =
								storageData.cs_cal_finished.includes(eventID);
						...
						reject({
							msg: errCode === 401 ? err_msg[1] : err_msg[0],
							is_error: true,
						});
					}
				})();
