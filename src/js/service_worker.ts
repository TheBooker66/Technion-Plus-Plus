...
async function TE_AutoLoginExternal() {
	try {
		const initialResponse = await XHR("https://moodle25.technion.ac.il/", "document", [
			"CourseNames",
			"UserText",
			"CourseLinks",
		]);
		...
		while (retryCount < MAX_RETRIES) {
			try {
				const finalResponse = await XHR("https://moodle25.technion.ac.il/", "document", [
					"CourseNames",
					"UserText",
					"CourseLinks",
				]);
...
async function TE_notification(message: string, silentEh: boolean, notificationId = "") {
	...
	// @ts-expect-error For some reason the type definitions don't include this property, even though it's supported.
...
async function TE_checkCalendarProp(calendarProp: string) {
	...
	try {
		const mainPageResponse = await XHR("https://moodle25.technion.ac.il/calendar/export.php", "document", [
			"SessionKey",
		]);
...
async function TE_webworkScan() {
	...
	for (const courseData of Object.values(storageData.webwork_cal_courses)) {
		...
		const step2 = await TE_webworkStep(step1[0], new URLSearchParams(step1[1] as string).toString());
		if (!step2) continue;

		const step3 = await TE_webworkStep(step2[0], new URLSearchParams(step2[1] as string).toString());
		if (!step3) continue;
...
async function TE_getWebwork(
	...
	for (let i = 0; i < courseUrlElements.length; i++) {
		...
		const ltiPage = await XHR(`https://moodle25.technion.ac.il/mod/lti/index.php?id=${courseID}`, "document", [
			"WebworkLinks",
		]);
...
export async function TE_updateInfo() {
	...
	const loginEnabledEh: boolean =
			(storageData.external_enable || storageData.enable_login) && storageData.quick_login,
		moodleCheckDueEh: boolean = loginEnabledEh && storageData.moodle_cal_enabled,
		webworkCheckDueEh: boolean =
			loginEnabledEh && storageData.webwork_cal_enabled && now - storageData.webwork_cal_update > EIGHT_HOURS;
