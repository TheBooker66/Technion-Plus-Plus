...
async function TE_setStorage(
	...
	if (chrome.runtime.lastError)
		console.error(
			`TE_bg_${callerName}: ${chrome.runtime.lastError.message}`,
		);
}
		...
		case "document":
			if (typeof browser !== "undefined" && browser) {
				...
				const actions = {
					...
					get HWList() {
						return (
							doc.activeElement?.innerHTML.split(
								"BEGIN:VEVENT",
							) ?? []
						);
					},
...
async function TE_AutoLoginNormal(headRequestEh: boolean) {
	try {
		const initialResponse = await XHR(
			"https://moodle25.technion.ac.il/auth/oidc/",
			"document",
			["CourseNames", "CourseLinks"],
			"",
			headRequestEh,
		);
		...
		params.append(
			"login_hint",
			`${loginDetails.username}@${loginDetails.email_server ? "campus." : ""}technion.ac.il`,
		);
		...
		for (let retryCount = 0; retryCount < MAX_RETRIES; retryCount++) {
			...
			if (currentTab.url === "https://moodle25.technion.ac.il/") {
				...
				console.log(
					`TE_auto_login: connection was made! At ${Date.now()}`,
				);
		...
		await chrome.tabs.remove(tabId);
		console.error(
			"Could not login to moodle, possibly wrong username/password (normal).",
		);
		return { response: "Error", responseURL: "" };
	} catch (err: any) {
		console.error(
			`TE_back_M_login: could not connect to moodle. {reason: ${err.message}} at ${Date.now()}`,
		);
		return { response: "Error", responseURL: "" };
	}
}
...
async function TE_AutoLoginExternal() {
	try {
		const initialResponse = await XHR(
			...
			"document",
			["CourseNames", "UserText", "CourseLinks"],
		);
		...
		const tab = await chrome.tabs.create({
			url: "https://moodle25.technion.ac.il/",
			active: false,
		});
		const tabId = tab.id as number;
		...
		await chrome.tabs.remove(tabId);
		console.error(
			"Could not login to moodle, possibly wrong username/password (external).",
		);
		return { response: "Error", responseURL: "" };
	} catch (err: any) {
		console.error(
...
async function TE_notification(
	message: string,
	silentEh: boolean,
	notificationId = "",
) {
	...
	const notificationOptions: chrome.notifications.NotificationCreateOptions =
		{
			...
			iconUrl: chrome.runtime.getURL(
				"../icons/technion_plus_plus/icon-128.png",
			),
			message: `${message}\n${timestamp}`,
		};

	if (typeof chrome.notifications.NotificationOptions?.silent !== "undefined")
	...
	if (chrome.runtime.lastError) {
		console.error(
			"TE_bg_notification_err: " + chrome.runtime.lastError.message,
		);
		...
		if (typeof browser !== "undefined") {
			...
			await chrome.runtime.sendMessage({
				mess_t: "audio notification",
				volume: storageData.notif_vol,
			});
			await chrome.offscreen.closeDocument();
		}
	}
...
async function TE_setBadge(errorEh: boolean) {
	...
	if (
		...
		await chrome.action.setBadgeBackgroundColor({
			color: errorEh ? [215, 0, 34, 185] : [164, 127, 0, 185],
		});
...
async function TE_alertNewHW(sourceIndex: number) {
	...
	await TE_setStorage(
		{ cal_seen: storageData.cal_seen | sourceInfo.flag },
		"HWA",
	);
	if (storageData.hw_alerts)
		await TE_notification(`יש לך מטלות חדשות ב${sourceInfo.name}!`, false);
}

async function TE_getCoursesMoodle(moodleData: { response: any }) {
	if (
		!moodleData.response["CourseNames"] ||
		moodleData.response["CourseNames"].length === 0
	) {
...
async function TE_checkCalendarProp(calendarProp: string) {
	...
	try {
		const mainPageResponse = await XHR(
			"https://moodle25.technion.ac.il/calendar/export.php",
			"document",
			["SessionKey"],
		);
		...
		const exportResponse = await XHR(
			mainPageResponse.responseURL,
			"document",
			["CalendarURL"],
			postBody,
		);
		...
		const userProp =
			"userid=" +
			calendarUrl.split("userid=")[1].split("&preset_what=all")[0];
...
async function TE_alertMoodleCalendar(
	seenStatus: number,
	calendarProp: string,
	maxEventId: number,
	filterToggles: {
	...
	try {
		...
		for (let i = 1; i < events.length; i++) {
			...
			const appealEh = filterToggles.appeals && summary.includes("ערעור"),
				zoomEh =
					filterToggles.zooms &&
					/(זום|Zoom|zoom|הרצא|תרגול)/.test(summary),
				attendanceEh =
					filterToggles.attendance &&
					/(נוכחות|Attendance|attendance)/.test(summary),
				reserveDutyEh =
					filterToggles.reserveDuty && summary.includes("מילואים");
			if (appealEh || zoomEh || attendanceEh || reserveDutyEh) continue;

			const summaryWords = summary.split(" ");
			if (
				summaryWords.at(-1) !== "opens" &&
				summaryWords.at(-1) !== "opens)"
			) {
				const eventUID = parseInt(
					eventText.split("UID:")[1].split("@moodle")[0],
				);
...
async function TE_csCalendarCheck(
	...
	try {
		...
		for (let i = 1; i < events.length; i++) {
			...
			const timeMatch = eventText.match(regexes.time)!.groups as {
				[key: string]: string;
			};
			...
			if (eventText.includes(".PHW")) newHWSet.delete(eventID);
			else {
...
async function TE_webworkStep(url: string | FormData, body = "") {
	...
	try {
		...
		const form: { action: string; data: { [key: string]: string } } | null =
			response.response["WebworkForm"];
		if (!form || !form.action) {
			return false;
		}
		...
		return webworkRegex.test(redirectUri.toString())
			? [actionUrl, formData]
			: false;
	} catch {
		return false;
...
async function TE_webworkScan() {
	...
	for (const courseData of Object.values(storageData.webwork_cal_courses)) {
		const step1 = await TE_webworkStep(
			`https://moodle25.technion.ac.il/mod/lti/launch.php?id=${(courseData as WebworkCourse).lti}`,
		);
		if (!step1) continue;

		const step2 = await TE_webworkStep(
			step1[0],
			new URLSearchParams(step1[1] as string).toString(),
		);
		if (!step2) continue;

		const step3 = await TE_webworkStep(
			step2[0],
		...
		const assignments: {
			...
			missions: { name: string; due: string }[] =
				page.response["WebworkMissions"];
		...
		for (let i = 0; i < missions.length; i++) {
			...
			const assignmentId = parseInt(
				(courseData as WebworkCourse).lti + "000" + i.toString(),
			);
			...
			assignments[assignmentId] = {
				h: hwName,
				ts: new Date(
					parseInt(dateMatch.year),
					parseInt(dateMatch.month) - 1,
					parseInt(dateMatch.day),
					parseInt(dateMatch.hour),
					parseInt(dateMatch.minute),
				).getTime(),
	...
	await TE_setStorage(
		{ webwork_cal_events: newWebworkCal, webwork_cal_update: Date.now() },
		"wwcfail_1",
	);
	if (foundNewAssignment) await TE_alertNewHW(2);
}

async function TE_getWebwork(
	moodleData: { response: any; responseURL: string },
	...
	for (let i = 0; i < courseUrlElements.length; i++) {
		const courseMatch = courseHeadings[i]
			.replace(semesterRegex, "")
			.match(courseRegex);
		...
		const ltiPage = await XHR(
			`https://moodle25.technion.ac.il/mod/lti/index.php?id=${courseID}`,
			"document",
			["WebworkLinks"],
		);
		const links: { text: string; href: string }[] =
			ltiPage.response["WebworkLinks"];
		for (const link of links) {
			if (webworkRegex.test(link.text)) {
				newWebworkCourses[courseID] = {
					name: cname.trim(),
					lti: link.href!.split("id=")[1],
				};
				break;
			}
		}
	...
	await TE_setStorage(
		{ webwork_cal_courses: newWebworkCourses },
		"webworkCourses",
	);
...
async function TE_single_download(link: string, name: string) {
	...
	if (chrome.runtime.lastError)
		console.error("TE_bg_dl: " + chrome.runtime.lastError.message);
}
...
async function TE_doDownloads(chunk: DownloadItem) {
	...
	if (chrome.runtime.lastError) {
		console.error(
			"TE_bg_download_fail: " + chrome.runtime.lastError.message,
		);
		const sizeError: string =
			JSON.stringify(storageData.dl_queue).length > 1e6
				...
				: "";
		await TE_notification(
			`שליחת הקבצים להורדה נכשלה. ${sizeError}\n`,
			true,
			"downloads",
		);
	} else {
...
async function TE_nextDownload() {
	...
	if (storageData.dl_current === 0 && storageData.dl_queue.length > 0) {
		...
		const downloadItem =
			currentQueueItem.list.shift() as DownloadItem["list"][0];
		const fullUrl =
			urlPrefixes[currentQueueItem.sys] +
			currentQueueItem.sub_pre +
			downloadItem.u;
		...
		chrome.downloads.download(
			...
			async (downloadId) => {
				if (chrome.runtime.lastError) {
					console.error(
						"TE_bg_dls: " + chrome.runtime.lastError.message,
					);
					console.log(
						` - filename: ${downloadItem.n}\n - url: ${fullUrl}`,
					);
					...
					await chrome.action.setIcon({
						path: "../icons/technion_plus_plus/icon-green.png",
					});
					setTimeout(async () => {
						await chrome.action.setIcon({
							path: "../icons/technion_plus_plus/icon-128.png",
						});
						setTimeout(async () => {
							await chrome.action.setIcon({
								path: "../icons/technion_plus_plus/icon-green.png",
					...
					await TE_setStorage({
						dl_current: storageData.dl_current,
						dl_queue: storageData.dl_queue,
					});
				}
			},
		);
...
export async function TE_updateVideosInfo(
	timestamp: number,
	callbacks: any = null,
) {
	...
	headers.append(
		"Authorization",
		"Basic Y291bHBsZWRseXNlcXVhbGxvbmVyd2FyOjZhODk1NTljMmQyYzFlNDViZTQyYzk3MDQ3N2E3MDRhMDkwNjg0ODg=",
	);
	headers.append("Content-Type", "application/json");

	try {
		const response = await fetch(
			"https://12041543-fd22-49b6-bf91-5fa9cf6046b2-bluemix.cloudant.com/tpvideos/v_Data%3Abff4cb5a16c3d92e443287a965d1f385",
			{ method: "GET", headers: headers },
		);
		const dbData = (await response.json()) as {
			_id: string;
			_rev: string;
			data: { [key: number]: RecordingCourse };
		};

		if (!dbData["data"] || !dbData["_id"]) {
			console.error(
				"TE_back_video_update_err: video-update bad request.",
			);
		...
		console.log(
			`TE_back: found ${coursesList.length} courses for videos-db (${timestamp})`,
		);
		await TE_setStorage(
			{
				videos_courses: coursesList,
				videos_data: videosData,
				videos_update: timestamp,
			},
			"uc",
		);
...
export async function TE_updateInfo() {
	...
	const THIRTY_DAYS = 2592e5,
		...
		TWO_DAYS = 1728e5,
		now = Date.now();
	if (storageData.videos_update < now - THIRTY_DAYS)
		await TE_updateVideosInfo(now);

	const loginEnabledEh: boolean =
			(storageData.external_enable || storageData.enable_login) &&
			storageData.quick_login,
		moodleCheckDueEh: boolean =
			loginEnabledEh && storageData.moodle_cal_enabled,
		webworkCheckDueEh: boolean =
			loginEnabledEh &&
			storageData.webwork_cal_enabled &&
			now - storageData.webwork_cal_update > EIGHT_HOURS;
	...
	if (moodleCheckDueEh || webworkCheckDueEh) {
		try {
			...
			if (moodleCheckDueEh) {
				...
				await TE_alertMoodleCalendar(
					storageData.cal_seen,
					storageData.moodle_cal_prop,
					storageData.moodle_cal_max,
			...
			if (webworkCheckDueEh) {
				await TE_getWebwork(
					moodleData,
					storageData.webwork_cal_courses,
				);
			}
		} catch (err) {
	...
	if (storageData.cs_cal_enabled && now - storageData.cs_cal_update > 1)
		await TE_csCalendarCheck(
			storageData.uidn_arr,
			storageData.cs_cal_pass,
			storageData.cs_cal_seen,
		);
...
async function TE_busAlertError() {
	await TE_notification(
		"התרחשה שגיאה בניסיון יצירת התראה לאוטובוס, אנא נסה שנית.\nשים לב: ההתראות הקיימות, במידה והיו, נמחקו.",
		false,
	);
...
async function TE_busAlertNow(arrivingBuses: BusLine[]) {
	...
	storageData.bus_alerts = storageData.bus_alerts.filter(
		(line: string) => !alertedLines.includes(line),
	);
	await TE_setStorage(
		{ bus_alerts: storageData.bus_alerts },
		"removeAlertedBuses",
	);
...
async function TE_checkBuses() {
	...
	if (chrome.runtime.lastError) {
		console.error(
			"TE_bg_checkBuses_err: " + chrome.runtime.lastError.message,
		);
		return;
	}
	...
	try {
		...
		const busesToAlert = realtimeData.filter(
			(bus) =>
				storageData.bus_alerts.includes(bus["Shilut"]) &&
				bus["MinutesToArrival"] <= storageData.bus_time,
		);
...
async function TE_startExtension() {
	await chrome.alarms.create("TE_update_info", {
		delayInMinutes: 1,
		periodInMinutes: 60,
	});
...
chrome.runtime.onMessage.addListener(async (message) => {
	switch (message.mess_t) {
		...
		case "TE_moodle_colour":
			await TE_sendMessageToTabs({
				mess_t: "TE_moodle_colour",
				angle: message.angle,
			});
			break;
...
chrome.downloads.onChanged.addListener(async (delta) => {
	...
	const finishDownload = async (isError = false) => {
		if (isError) {
			const systemName =
				["moodle", "panopto", "GR++", "webcourse"][
					storageData.dl_queue[0]?.sys
				] ?? "unknown";
			console.error(`TE_dlFailed ${delta.id} : ${systemName}`);
		}

		if (storageData.dl_queue[0]?.list.length === 0)
			storageData.dl_queue.shift();
