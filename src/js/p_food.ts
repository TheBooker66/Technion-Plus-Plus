...
(function () {
	...
	const popup = new CommonPopup("מסעדות פתוחות בחיפה ובנשר", [""], document.title);
	chrome.storage.local.get({allow_timings: false}, storageData => {
		...
		popup.XHR("../resources/food.csv", "csv").then(res => {
			...
			restaurants.forEach((item) => {
				...
				item['working_hours'] = (item['working_hours'] as string).replace('"{', '{').replace('}"', '}');
				item['working_hours'] = JSON.parse(item['working_hours']);

				// Parse the phone number (Israeli format)
				item['phone'] = (item['phone'] as string).replace('+972 ', '0').replace(/-/g, '');
			});
			...
			const date = {
				day: currentDate.getDay().toString(), hour: currentDate.getHours().toString(),
				minutes: currentDate.getMinutes().toString(), time: "",
			};
			date.hour = parseInt(date.hour) < 10 ? "0" + date.hour : date.hour;
			date.minutes = parseInt(date.minutes) < 10 ? "0" + date.minutes : date.minutes;
			...
			for (let i = 0; i < restaurants.length; i++) {
				...
				if (end > date.time && date.time > start) {
					...
					restaurants[i].distance =
						Math.sqrt(Math.pow(technionLoc.latitude - parseFloat(restaurants[i].latitude as string), 2)
							+ Math.pow(technionLoc.longitude - parseFloat(restaurants[i].longitude as string), 2));
				} else {
					restaurants.splice(i, 1);
					i--;
				}
