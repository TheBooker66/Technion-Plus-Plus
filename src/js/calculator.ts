(async function () {
	...
	function handleStorageError(operation: string) {
		if (chrome.runtime.lastError) {
			console.error(
				`TE_calculator_${operation}: ${chrome.runtime.lastError.message}`,
			);
			return true;
		}
		return false;
	...
	function calculateTableStats(tableSelector: string) {
		const gradeElements = document.querySelectorAll(
			...
			pointsElements = document.querySelectorAll(
				`${tableSelector} .points`,
			) as NodeListOf<HTMLInputElement>;
		let sum = 0,
			totalPoints = 0,
			pointsForAverage = 0,
		...
		for (let i = 0; i < gradeElements.length; i++) {
			...
			if (gradeElements[i].tagName === "INPUT") {
				...
				pointsWithPassingGrade +=
					grade === "עובר" || grade === "פטור עם ניקוד" ? points : 0;
			}
		}
	...
	function updateSelectedCoursesStats() {
		const selectedTableCells =
				document.querySelectorAll("#selected_tbl td"),
			selectedStats = calculateTableStats(".selected");

		if (selectedStats.count === 0)
			selectedTableCells[1].textContent = "לא בחרתם אף קורס, נו באמת...";
		else if (selectedStats.count === 1)
			selectedTableCells[1].textContent = "קורס 1";
		else
			selectedTableCells[1].textContent = `${selectedStats.count} קורסים`;

		selectedTableCells[3].textContent = selectedStats.avg;
		selectedTableCells[5].textContent = selectedStats.points.toString();
		selectedTableCells[7].textContent =
			selectedStats.points_passed.toString();
	}
	...
	function handleSelectAllClick(event: Event) {
		...
		document
			.querySelectorAll("#grades_list tbody tr")
			.forEach((rowElement) => {
				const checkbox = rowElement.querySelector(
					"input[type='checkbox'].select_course",
				) as HTMLInputElement;
	...
	function updateAllStats() {
		const allGradesStats = calculateTableStats("#grades_list"),
			allPointsElements = document.querySelectorAll(
				"#grades_list tr .points",
			) as NodeListOf<HTMLInputElement>,
			passingPointsElements = document.querySelectorAll(
				"#grades_list tr:not(.failed) .points",
			) as NodeListOf<HTMLInputElement>;
		const totalPoints = Array.from(allPointsElements).reduce(
		...
		document.getElementById("total_points")!.textContent =
			allGradesStats.points_passed.toString();
	...
	function createCourseRowElement(
		...
		const rowElement = templateContent.querySelector(
			"tr",
		) as HTMLTableRowElement;

		rowElement.classList.add("animate");
		if (
			parseInt(courseData.grade.toString()) < 55 ||
			courseData.grade === "נכשל"
		)
			rowElement.classList.add("failed");
		...
		(cellElements[2].querySelector(".points") as HTMLInputElement).value =
		...
		const gradeInput = cellElements[3].querySelector(
			".grade",
		) as HTMLInputElement;
		const editButton = cellElements[3].querySelector(
			"button",
	...
	async function handleGradesListClick(event: PointerEvent) {
		...
		const courseNum = (
			rowElement.querySelector("td:first-child") as HTMLTableCellElement
		).textContent;
		const courseData = allGrades.find(
			(course: CalculatorCourse) => course.num === courseNum,
		);
		...
		const gradeInput = rowElement.querySelector(
			".grade",
		) as HTMLInputElement;
		...
		switch (target.textContent) {
			...
			case "אישור": {
				...
				if (
					...
					setTimeout(
						() => gradeInput.classList.remove("failed"),
						1000,
					);
					return;
				}
				...
				if (!rowElement.classList.contains("temporary")) {
					...
					document
						...
						?.querySelector("tbody")
						?.prepend(
							createCourseRowElement(courseData, "ignore_list"),
						);
				}
				break;
			}
			...
			case "מחק":
				...
				document
					.getElementById("ignore_list")
					?.querySelector(`#course_${courseNum}`)
					?.closest("tr")
					?.remove();
				document
					...
					?.querySelector("tbody")
					?.prepend(
						createCourseRowElement(courseData, "grades_list"),
					);
				break;
		}
		updateAllStats();
	...
	async function handleIgnoreListClick(event: PointerEvent) {
		...
		switch (target.textContent) {
			case "שחזר":
				...
				break;
			case "מחק": {
				const sureEh = confirm(
					"האם אתם בטוחים שברצונכם למחוק את הקורס הזה?",
				);
				if (!sureEh) return;
				...
				document
					...
					?.closest("tr")
					?.remove();
				const updatedGrades = allGrades.filter(
					(course: CalculatorCourse) => course.num !== courseNum,
				);
	 ...
	 */
	function validateCourseInput(course: CalculatorCourse): {
		isValid: boolean;
		message: string;
	} {
		if (!/^[0-9A-Za-z]{8}$/.test(course.num.toString())) {
			return {
				isValid: false,
				message: "מספר הקורס חייב להיות בן 8 תווים (ספרות או אותיות).",
			};
		}
		...
		if (
			...
			parseInt(course.points.toString()) < 0
		) {
			return {
				isValid: false,
				message: "נא לכתוב מספר נקודות זכות תקין (מספר חיובי).",
			};
		}
		if (
			!course.binary &&
			...
			return {
				isValid: false,
				message: "נא להזין ציון מספרי תקין בין 0 ל-100.",
			};
		}
		if (
			course.binary &&
			![
				...
				"פטור עם ניקוד",
				"פטור ללא ניקוד",
			].includes(course.grade.toString())
		) {
			return {
				isValid: false,
				message: "נא לבחור 'עובר' או 'נכשל' עבור ציון בינארי.",
			};
		}
		if (
			course.year &&
			(Number.isNaN(Number(course.year)) ||
				course.year < 1912 ||
				course.year > 65537)
		) {
	...
	function setUpButtons() {
		const binary_checkbox = document.getElementById(
				"binaryEh",
			) as HTMLInputElement,
			gradeInput = document.getElementById("grade") as HTMLInputElement,
			gradeLabel = document.getElementById(
				"grade_label",
			) as HTMLLabelElement,
			binaryGradeInput = document.getElementById(
				"binary_grade",
			) as HTMLInputElement,
			binaryGradeLabel = document.getElementById(
				"binary_grade_label",
			) as HTMLLabelElement;
		...
		const addGradeForm = document.getElementById(
			"add_grade_form",
		) as HTMLFormElement;
		addGradeForm.addEventListener("submit", async (event) => {
			...
			const newCourse: CalculatorCourse = {
				...
				binary: binaryEh === "on",
				grade:
					binaryEh === "on" ? binary_grade.trim() : parseFloat(grade),
			...
			if (
				storageData.grades.some(
					(course) => course.num === newCourse.num,
				)
			) {
			...
			const latestYear = storageData.grades.reduce(
				(acc: number, course: CalculatorCourse) =>
					Math.max(acc, course.year),
				1912,
			);
			const latestSemesterOrder = storageData.grades
				.filter(
					(course: CalculatorCourse) => course.year === latestYear,
				)
				.reduce(
					(maxOrder: number, course: CalculatorCourse) =>
						Math.max(maxOrder, semesterOrder[course.semester]),
					0,
				);
			...
			if (
				newCourse.year === latestYear &&
				newCourse.semester === latestSemester
			) {
		...
		(
			document.getElementById("export") as HTMLInputElement
		).addEventListener("click", () => {
			if (
				document.querySelectorAll("#grades_list tbody tr").length === 0
			) {
		...
		(
			document.getElementById("import") as HTMLInputElement
		).addEventListener("click", () => {
			...
			input.onchange = (event) => {
				...
				function smallValidate(
					...
					if (!validationResult.isValid) {
						console.warn(
							`Validation failed for row: ${line} - ${validationResult.message}`,
						);
						return false;
					}
					...
					if (
						currentStoredGrades.some(
							(c: CalculatorCourse) => c.num === course.num,
						) ||
						newCourses.some(
							(c: CalculatorCourse) => c.num === course.num,
						)
						...
						console.log(
							`Skipping duplicate course during import: ${course.num}`,
						);
						return false;
					}
					return true;
				...
				if (file.name.endsWith(".csv")) {
					...
					reader.onload = async (event) => {
						...
						const pdfjs = await import(
							chrome.runtime.getURL(pdfjsPath + "pdf.min.mjs")
						);
						...
						lines.forEach((line) => {
							...
							if (!parts) {
								console.warn(
									"Skipping malformed row (regex didn't match) during PDF import:",
									line,
								);
								return;
							}
		...
		document
			.getElementById("delete_grades")
			?.addEventListener("click", async () => {
				const sureEh = confirm(
					"האם אתה בטוח שברצונך למחוק את כל הציונים מזיכרון התוסף? פעולה זו אינה הפיכה!",
				);
				if (!sureEh) return;
				...
				if (!handleStorageError("delete_all_grades")) {
					...
					alert(
						"אירעה שגיאה בעת מחיקת הציונים. אנא רעננו את העמוד ונסו שנית.",
					);
			});

		(
			document.getElementById("grades_list") as HTMLTableElement
		).addEventListener("click", handleGradesListClick);
		(
			document.getElementById("ignore_list") as HTMLTableElement
		).addEventListener("click", handleIgnoreListClick);

		(
			document.getElementById("select_all_courses") as HTMLInputElement
		).addEventListener("change", handleSelectAllClick);
	}
	...
	async function renderAllCourses() {
		...
		const gradesToPersist = allGrades.map((course: CalculatorCourse) => {
			course.selected =
				course.semester === latestSemester &&
		...
		const lists = {
			grades_list: storageData.grades.filter(
				(course: CalculatorCourse) => !course.perm_ignored,
			),
			ignore_list: storageData.grades.filter(
				(course: CalculatorCourse) => course.perm_ignored,
			),
		};
		...
		for (const listKey in lists) {
			document
				.getElementById(listKey)!
				.querySelector("tbody")!.innerHTML = "";
			const fragment = document.createDocumentFragment();
			const sortedData: CalculatorCourse[] = [
				...lists[listKey as "grades_list" | "ignore_list"],
			].sort(
