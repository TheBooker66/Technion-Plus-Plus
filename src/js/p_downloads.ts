...
(async function () {
	...
	async function updateCurrentDownload() {
		...
		if (storageData.dl_current !== 0)
			chrome.downloads.search(
				{ id: storageData.dl_current },
				(downloads) => {
					if (downloads[0].filename) {
						updateTextContent(
							currentDownloadElements[0],
							downloads[0].filename,
						);
						let statusText = downloads[0].paused ? "מושהה" : "פעיל";
						updateTextContent(
							currentDownloadElements[1],
							statusText,
						);
						if (0 < downloads[0].totalBytes) {
							statusText +=
								" - " +
								(downloads[0].bytesReceived / 1048576).toFixed(
									1,
								) +
								"/" +
								(downloads[0].totalBytes / 1048576).toFixed(1) +
								"MB";
							updateTextContent(
								currentDownloadElements[1],
								statusText,
							...
							updateProgressDisplay(
								currentDownloadElements[2],
								(
									(100 * downloads[0].bytesReceived) /
									downloads[0].totalBytes
								).toFixed(1),
							);
						} else
							updateProgressDisplay(
	...
	async function updateDownloadQueue() {
		while (queueList.firstChild)
			queueList.removeChild(queueList.lastChild as Node);
		const storageData = (await chrome.storage.local.get({
			dl_queue: [],
		})) as StorageData;
		let itemCount = 0;
		storageData.dl_queue.forEach((downloadEntry: DownloadItem) => {
			for (let i = 0; i < downloadEntry.list.length; i++) {
				const file = downloadEntry.list[i],
					listItem = (
						popup
							.loadTemplate("dl_item")
							.cloneNode(true) as HTMLElement
					).querySelector(".list_item") as HTMLDivElement;
				(
					listItem.querySelector(".dl_name") as HTMLElement
				).textContent = file.n;
				(listItem.querySelector(".dl_from") as HTMLImageElement).src =
					"../icons/" +
					["moodle.svg", "panopto.svg", "cs.png", "cs.png"][
						downloadEntry.sys
					];
				(
					listItem.querySelector(".remove") as HTMLImageElement
				).addEventListener("click", async () => {
					const fileIndex = downloadEntry.list.indexOf(file);
					downloadEntry.list.splice(fileIndex, 1);
					if (downloadEntry.list.length === 0)
						storageData.dl_queue.splice(
							storageData.dl_queue.indexOf(downloadEntry),
							1,
						);
					...
					if (!queueList.firstChild) {
						...
						).textContent = "אין קבצים בהמתנה להורדה על ידי התוסף.";
						(queueList.firstChild! as HTMLElement).style.padding =
							"8px";
					}
				});
		...
		if (itemCount === 0) {
			queueList.appendChild(document.createElement("span")).textContent =
	...
	const currentDownloadElements = document.querySelectorAll(
			"#current span",
	...
	chrome.downloads.onCreated.addListener(() =>
		setTimeout(updateDownloadQueue, 1e3),
	);
	chrome.downloads.onChanged.addListener((changes) => {
		if (changes.state || changes.paused) updateDownloadQueue();
	});
	(document.getElementById("pause") as HTMLInputElement).addEventListener(
		"click",
		async () => {
			const storageData = (await chrome.storage.local.get({
				dl_current: 0,
	...
	(document.getElementById("cancel") as HTMLInputElement).addEventListener(
		"click",
		async () => {
			const storageData = (await chrome.storage.local.get({
				dl_current: 0,
			...
			if (storageData.dl_current !== 0)
				await chrome.downloads.cancel(storageData.dl_current);
		},
	);
	(document.getElementById("cancelAll") as HTMLInputElement).addEventListener(
		"click",
		async () => {
			const storageData = (await chrome.storage.local.get({
				dl_current: 0,
