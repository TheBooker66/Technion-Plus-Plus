export class CommonPopup {
	...
	constructor(title: string = "", css_list: string[] = [], context: string) {
		...
		void this.useTemplatesFile("popup", (documentContext: Document) => {
			for (let i = 0; i < css_list.length; i++) {
				const linkTemplate = this.loadTemplate(
					"head-stylesheets",
					documentContext,
				);
				...
				if (element)
					element.setAttribute(
						"href",
						"../css/p_" + css_list[i] + ".css",
					);
				document.head.appendChild(linkTemplate);
			}

			const wrapper = document.querySelector(".wrapper") as HTMLElement;
			const main_content = wrapper.querySelector(
				".main-content",
			) as HTMLElement;
			wrapper.insertBefore(
				this.loadTemplate("header-koteret", documentContext),
				main_content,
			);
			if (this.title !== "") {
				const newTemplateDocument = this.loadTemplate(
					"header-title",
					documentContext,
				);
				const element = newTemplateDocument.querySelector(
					"span:not(#returnHome)",
				);
	...
	useTemplatesFile = async (
		templateFileName: string,
		callback: (documentContext: Document) => void,
	) => {
		const template = await fetch(
			"../html/templates/" + templateFileName + ".html",
		);
	...
	buttonsSetup() {
		...
		document
			...
			?.addEventListener(
				"click",
				() => (window.location.href = "../html/p_about.html"),
			);
		if (this.title !== "")
			document
