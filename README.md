...
- [פ'יצרים – מה התוסף עושה?](#פיצרים--מה-התוסף-עושה)
    ...
    - [ארגונית פלוס](#ארגונית-)
    - [מחשבון ממוצע ציונים אקדמי](#מחשבון-ממוצע-ציונים-אקדמי)
    - [שיפורים במערכת מודל](#שיפורים-במערכת-מודל)
...
התוסף _Technion Plus Plus_ (ובקצרה _++Technion_ (ובעברית _טכניון++_)) הוא תוסף לדפדפנים על בסיס Chromium (למשל Google
Chrome, Opera, Brave, Microsoft Edge, ועוד), ואולי בעתיד גם FireFox, שמטרתו היא לעשות סדר בבלגן, להוסיף תכונות משודרגות
ולהפוך את השימוש במערכות הטכניון השונות לנוח ויעיל יותר.
...
<img align="left" width="200" src="/assets/popup.png"  alt="החלונית הראשית של התוסף"/>
...
לאחר התקנת התוסף, יש להגדיר את פרטי ההתחברות בהגדרות התוסף, ומכאן ואילך התוסף כבר ידאג לחבר אתכם אוטומטית אל המערכות הנ"
ל. ניתן לבטל בלחיצת כפתור את החיבור האוטומטי במקרה הצורך.
...
- ניתן לסמן מטלות ככאלה שבוצעו, מטלות כאלו יועברו לכרטיסיית "מטלות שהושלמו".
...
- בפעם הראשונה צריך להזין אותם ידנים, אולם לאחר מכן המחשבון זוכר את הציונים כאילו הם תמיד היו שם.
...
- כרטיסיית "קישורים שימושיים" בתוסף: אפגרייד, BookMe, לוח שנה אקדמי, מפה רשמית של הטכניון, הורדת אופיס 365 טכניוני
...
יש להשתמש בתוסף אך ורק על ידי הורדתו מחנות התוספים הרשמית של דפדפן Google Chrome, גם אם משתמשים בדפדפנים אחרים או
בדפדפנים המבוססים עליו (למשל דפדפני Opera ו־Microsoft Edge). השימוש בתוסף מהווה אישור לכך שהמשתמש קרא את מדיניות הפרטיות
המפורטת בדף זה והסכים לה.

התוסף, טכניון פלוס פלוס, אוסף רק את הפרטים הבאים שהמשתמש מזין לו מרצונו החופשי: העדפות כלליות, אימייל טכניוני, סיסמה
לאימייל הטכניוני, ו/או מספר תעודת זהות. אם המידע נמחק מהתוסף (על ידי דף ההגדרות) הוא נמחק כליל ולא נשמר בשום מקום. פרטי
המשתמש וההגדרות שבשימוש התוסף נשמרים בזיכרון הדפדפן של המשתמש בלבד ולא נשלחים לאף שרת (למעט שרתים רשמיים של הטכניון)
וכמובן שלא לאף צד ג'. יש להבהיר שוב כי התוסף לא שולח שום מידע לשום מקום; כל המידע נותר לוקאלית על המחשב של המשתמש.
...
- יש לציין כי חנות התוספים ממנה הורד התוסף יכולה לאסוף מידע הנוגע למשתמשי התוסף, וזאת בהתאם למדיניות הפרטיות שלה: מערכת
  ...
  המידע שנאסף על ידי חנויות התוספים.
...
- המידע האישי שנאסף על ידי התוסף לא מועבר לאף גורם בשום מקרה, למעט פרטי החיבור של המשתמש (משתמש טכניוני וסיסמת המשתמש,
  או מספר זהות וסיסמת יומן GR++) המועברים למערכות הטכניון לשם חיבור המשתמש למערכות הטכניוניות או באיסוף מידע לשם הצגת
  מטלות בית.
...
יש לכם עצה? לא הצלחתם לדווח על תקלה או לבקש פי'צר חדש במאגר הקוד? סתם רוצים לדבר?? מוזמנים ליצור קשר עם המפתח הקודם
במייל [maorlevy.ext@gmail.com](mailto:maorlevy.ext@gmail.com) ועם המפתח הנוכחי
במייל [ethan.amiran@gmail.com](mailto:ethan.amiran@gmail.com), או להשתמש בכפתור בכרטיסיית האודות בתוסף על מנת לשלוח מייל
בקלות. אם בכלל הגעתם לעמוד הזה תשלחו מייל, סתם כדי שאדע שההשקעה בו הייתה שווה משהו.

### תרומה

אם משהו השתלט עליכם, יש לכם כמה שקלים ספייר ואתם מסיבה כלשהי רוצים לתרום למפתח, תוכלו לתרום למפתח
המקורי, [מאור](https://www.paypal.me/maorlevy/15ILS), או למפתח הנוכחי, [איתן](src/resources/bit_donation.jpg) קוד QR של
ביט.
