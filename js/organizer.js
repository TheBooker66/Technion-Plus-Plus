...
export class OrganizerPopup extends CommonPopup {
	...
	XHR(a, b, c) {
		return this.XHR_bigger(a, b, void 0 === c ? "" : c)
	}
}
...
function insertAssignments(a, b) {
	...
	var d = (e, g, l) => {
		...
		if ("ua" == e.sys)
			...
			m[1].addEventListener("click", n => toggle(g, e.toggleFunc, 1));
			m[2].addEventListener("click", n => toggle(g, e.toggleFunc, 0));
			m[0].addEventListener("click", n => openAssignment(g, e.goToFunc));
			g.querySelector(".assignment_header").addEventListener("click", n => openAssignment(g, e.goToFunc));
...
function form_submit() {
	0 == form.subject.value.length ? alert("חובה למלא נושא למטלה") : form.no_end.checked || "" !== form.end_time.value ? !form.no_end.checked && form.end_time.valueAsNumber < Date.now() ? alert("תאריך הסיום שבחרת כבר עבר, נא לבחור תאריך סיום חדש") :
		chrome.storage.local.get({user_agenda: {}}, a => {
			...
			50 < Object.keys(b).length ? alert("לא ניתן ליצור יותר מ־50 מטלות משתמש.") :
				// לא ניתן ליצור יותר מ-50 מטלות משתמש.
	...
	// חובה לבחור תאריך סיום או לסמן את "ללא תאריך סיום"
}
...
(function () {
	function C(c, d) {
		return () => new Promise((e, b) => {
			...
			TE_forcedAutoLogin(!0).then(() => l.XHR(m, "document").then(a).catch(b)).catch(b)
		})
	}
