...
class CommonPopup {
	constructor(a) {
		...
		this.moduleName = window.location.pathname.split(".")[0].split("/")[1]
	}

	useTemplatesFile(a, b) {
		fetch("./templates/" + a + ".html").then(c => c.text()).then(c => {
	...
	buttonsSetup() {
		...
		document.getElementById("goToAbout").addEventListener("click", () => window.location.href = "./about.html");
