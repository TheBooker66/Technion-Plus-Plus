...
(function () {
	...
	e.css_list = ["main"];
	e.popupWrap();
	navigator.appVersion.includes("Android") || chrome.browserAction.getBadgeBackgroundColor({}, c => {
		chrome.browserAction.getBadgeText({}, h => {
			215 == c[0] && 0 == c[1] && 34 == c[2] && "!" == h && (chrome.browserAction.setBadgeText({text: ""}), document.getElementById("bus_error").style.display = "block")
		})
	});
	...
	var r = {
		type: "popup", focused: !0, state: "normal", url: "organizer.html", height: Math.min(window.screen.height -
	...
	for (let c = 0; c < t.length; c++) [1, 2, 3].includes(c) || t[c].addEventListener("click", () => {
		window.location.href = t[c].id + ".html"
	});
