...
(function () {
	...
	function m() {
		for (var a = document.getElementById("username").value, c = document.getElementById("password").value, d, b, k; d = l(f(c)), b = d[0].toString(), d = d[1], k = b.substring(Math.floor(b.length / 2), b.length), b = b.substring(0, Math.floor(b.length / 2)), f(h(b + k, d)).toString() !== c.toString();) ;
		...
		chrome.storage.local.set({
			...
			chrome.runtime.lastError ? (console.log("TE_opt: " + chrome.runtime.lastError.message), e.textContent = "\u05e9\u05d2\u05d9\u05d0\u05d4 \u05d1\u05e9\u05de\u05d9\u05e8\u05ea \u05d4\u05e0\u05ea\u05d5\u05e0\u05d9\u05dd, \u05d0\u05e0\u05d0 \u05e0\u05e1\u05d4 \u05e9\u05e0\u05d9\u05ea!") :
				setTimeout(function () {
					e.textContent = ""
				}, 2E3)
		});
		p && r && n ? chrome.runtime.getBackgroundPage(e => e.TE_updateInfo()) : chrome.browserAction.getBadgeBackgroundColor({}, e => {
			chrome.browserAction.getBadgeText({}, C => {
				164 == e[0] && 127 == e[1] && 0 == e[2] && "!" == C && chrome.browserAction.setBadgeText({text: ""})
			})
		})
	}
	...
	document.getElementById("bug_report").addEventListener("click", () => {
		var a = {
				ad: "thebooker666@gmail.com", //"maorlevy.ext@gmail.com",
	...
	document.getElementById("try_vol").addEventListener("click", () => {
		...
		a.src = chrome.runtime.getURL("notification.mp3");
		a.play()
	});
