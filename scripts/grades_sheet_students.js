...
(function () {
	...
	function T(a, b, f, d) {
		...
		fetch(chrome.extension.getURL("templates/grades_sheet.html")).then(c => c.text()).then(c => {
	...
	if (2 != q) O("a", "no-print", {
		...
		for (var F = document.querySelectorAll("table.table-sm.table-striped > tbody"), V = document.querySelectorAll("table.table-sm > thead > tr:first-of-type > th"), w = F.length - 1; 0 <= w; w--) {
			...
			} else for (t = B.length - 1; 0 <= t; t--) if (m = B[t].querySelectorAll("td"), l = m[3].textContent.replace("*", "").trim(), !("-" == l || "\u05e2\u05d1\u05e8" != l && "\u05dc\u05d0 \u05d4\u05e9\u05dc\u05d9\u05dd" !=
				l && isNaN(parseInt(l)))) {
				"" == r && (r = x);
				var L = parseInt(l), G = parseFloat(m[2].textContent), W = m[1].textContent,
					M = m[0].textContent.trim(), C = W.trim();
				h.total_points_tried += G;
				h.points_succeed += 55 <= L || "\u05e2\u05d1\u05e8" == l ? G : 0; // עבר
				m = {name: C, num: M, points: G, grade: L, sem: x};
				if ("\u05dc\u05d0 \u05d4\u05e9\u05dc\u05d9\u05dd" == l) // לא השלים
					m.grade = l, J.push(m);
				else {
					"\u05e2\u05d1\u05e8" == l && (m.grade = l); // עבר
					var N = !0;
